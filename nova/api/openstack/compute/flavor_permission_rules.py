...
class FlavorPermissionRulesController(wsgi.Controller):
    ...
    @staticmethod
    def _resolve_flavor(context, flavor_ref: str) -> Flavor:
    ...
    def index(self, req: wsgi.Request) -> dict[str, ty.Any]:
        ...
        filter_by_context_project = not access_domain
        ...
        filter_by_context_domain = not access_all
        domain_id = req.GET.get('domain_id')
        if domain_id and not access_all:
            raise webob.exc.HTTPForbidden(
                explanation="Policy doesn't allow filtering by domain_id")

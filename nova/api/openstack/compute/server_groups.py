...
class ServerGroupController(wsgi.Controller):
    ...
    def update(self, req, id, body):
        ...
        LOG.info('called update with add_members: %s and remove_members %s',
                 ', '.join(members_to_add), ', '.join(members_to_remove))
        ...
        if members_to_remove:
            objects.InstanceGroup.remove_members(context, id,
                                                 members_to_remove)
        if members_to_add:
            try:
        ...
        # DB. This does not update the local session for deleted members, so we
        # have to drop them explicitly.
        sg.refresh()
        sg.members = [m for m in sg.members if m not in members_to_remove]

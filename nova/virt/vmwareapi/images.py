...
def fetch_image_stream_optimized(context, instance, session, vm_name,
    ...
    if not imported_vm_ref:
        raise vexc.VMwareDriverException(
            "Could not import image %s within %d "
            "attempts." % (vm_name, max_attempts))
    ...
    LOG.info("The imported VM '%s' was unregistered", vm_name,
        instance=instance)
...
def _wait_for_import_task(session, vm_ref):
    ...
    try:
        task_collector = session._call_method(
            session.vim, "CreateCollectorForTasks",
            session.vim.service_content.taskManager, filter=task_filter_spec)

        while True:
            page_tasks = session._call_method(session.vim, "ReadNextTasks",
                                              task_collector, maxCount=10)
            if len(page_tasks) == 0:
                break
            ...
            for ti in page_tasks:
                if ti.descriptionId == "ResourcePool.ImportVAppLRO":
                    try:
                        ...
                        LOG.debug("Awaiting previous import on VM "
                                  "%s failed with %s", vm_ref, e)

                    return waited
    finally:
        if task_collector:
            session._call_method(session.vim,
                                 "DestroyCollector", task_collector)
    return waited

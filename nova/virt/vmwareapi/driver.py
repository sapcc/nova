...
class VMwareVCDriver(driver.ComputeDriver):
    ...
    def get_console_output(self, context, instance):
        ...
        if CONF.vmware.serial_log_uri:
            try:
                ...
                LOG.exception('Unable to obtain serial console log for VM '
                              '%(vm_uuid)s with server details: %(server)s.',
    ...
    def get_cluster_metrics(self):
        ...
        for c_host in cluster_data['host'][0]:
            ...
            self.memory_usage += (
                props['summary.quickStats'].overallMemoryUsage)
        ...
        self.cluster_metrics['memory_total'] = (
            float(cluster_data['summary'].totalMemory / units.Mi))
        self.cluster_metrics['memory_used'] = self.memory_usage
        self.cluster_metrics['memory_free'] = (
            self.cluster_metrics['memory_total'] -
            self.cluster_metrics['memory_used'])
        self.cluster_metrics['datastore_total'] = (
            float(self.datastore_total / units.Gi))
        self.cluster_metrics['datastore_used'] = (
            float((self.datastore_total - self.datastore_free_space) /
            units.Gi))
        self.cluster_metrics['datastore_free'] = (
            self.cluster_metrics['datastore_total'] -
            self.cluster_metrics['datastore_used'])

        perc = (float(self.cluster_metrics['cpu_used']) /
            float(self.cluster_metrics['cpu_total']))
        ...
        perc = (float(self.cluster_metrics['datastore_used'] / units.G) /
            float(self.cluster_metrics['datastore_total'] / units.G))

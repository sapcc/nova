...
class VMwareVMOps(object):
    ...
    def build_virtual_machine(self, instance, context, image_info,
        ...
        config_spec = self._get_vm_config_spec(instance, image_info,
                                               datastore, network_info,
    ...
    def _fetch_image_from_other_datastores(self, vi):
        ...
        dc_other_datastores = [ds for ds in dc_all_datastores
                                if dict(ds.ref) != dict(vi.datastore.ref)]
        ...
        for ds in dc_other_datastores:
            ...
            if other_templ_vm_ref:
                ...
                clone_spec = vm_util.clone_vm_spec(
                    client_factory, rel_spec, template=True)
    ...
    def _fetch_image_if_missing(self, context, vi):
        ...
        with lockutils.lock(str(vi.cache_image_path),
            ...
            image_available = ds_util.file_exists(self._session, ds_browser,
                                                  vi.cache_image_folder,
            ...
            if not image_available:
                ...
                if tmp_dir_loc:
                    self._delete_datastore_file(
                        str(tmp_dir_loc), vi.dc_info.ref)
    ...
    def _create_image_template(self, context, vi, extra_specs):
        ...
        for i in range(max_attempts):
            try:
                templ_vm_ref = self.build_virtual_machine(
                    vi.instance,
                    context,
                    vi.ii,
                    vi.dc_info,
                ...
                vm_util.mark_vm_as_template(self._session, vi.instance,
                                            templ_vm_ref)
                ...
                with excutils.save_and_reraise_exception(
                    ...
                    try:
                        ...
                        LOG.error('Cleaning up VM template for image failed '
                                  'with error: %s',
    ...
    def _get_vm_template_for_image(self, context, instance,
        ...
        with lockutils.lock(templ_instance.uuid,
            ...
            # the image so that it can be extended during instantiation
            # (after clone) to match the flavor
            ...
            if not templ_vm_ref:
                templ_vm_ref = self._create_image_template(context, vi,
                                                           extra_specs)
    ...
    def _create_instance_from_image_template(self, context, client_factory,
        ...
        root_vmdk_info = vm_util.get_vmdk_info(
            self._session, vm_ref, uuid=vi.instance.uuid)
        self._extend_if_required(
            vi.dc_info, vi.ii, vi.instance, root_vmdk_info.path)
        ...
        vm_util.append_vif_infos_to_config_spec(
            client_factory, reconfig_spec, vif_infos, extra_specs.vif_limits)
    ...
    def _create_vm_clone(self, instance, vm_ref, snapshot_ref, dc_info,
        ...
        vm_clone_task = self._session._call_method(
            ...
            vm_ref,
            folder=self._get_project_folder(
                dc_info, project_id=instance.project_id, type_='Images'),
            name=vm_name,
            spec=clone_spec)
        self._session._wait_for_task(vm_clone_task)
    ...
    def snapshot(self, context, instance, image_id, update_task_state):
        ...
        try:
            ...
            snapshot_vm_ref = self._create_vm_clone(
                instance, vm_ref,
                snapshot_ref, dc_info,
                disk_move_type=disk_move_type,
                image_id=image_id, disks=[vmdk])
    ...
    def _parse_change_set(change_set):
        for change in change_set:
            ...
            if change.name == "config.managedBy":
                yield change.name, val and (
                    getattr(val, "extensionKey") == constants.EXTENSION_KEY)
            else:
                ...
                for update in update_set.filterSet[0].objectSet:
                    ...
                    if update.kind == "leave":
                        ...
                        LOG.debug("%s.%s.%s: %s", vm_ref["_type"],
                            update.kind, vm_ref.value, changes)

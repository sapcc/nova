...
class VMwareVMOps(object):
    ...
    def migrate_disk_and_power_off(self, context, instance, dest, flavor,
        ...
        """
        LOG.debug("Start migrating to dest=%s", dest, instance=instance)
        ...
        vm_util.vm_device_change(self._session, vm_ref, device_change)
    ...
    def _do_confirm_migration(self, context, instance, migration):
        ...
        dest = self._decode_host_addr(migration.dest_host)
        remote = self._is_remote(dest)
        session = self._create_dest_session(dest) if remote else self._session

        # update the instanceUuid of the dest VM
        vm_ref = vm_util._get_vm_ref_from_vm_uuid(session, migration.uuid)
        vm_util.change_vm_instance_uuid(session, vm_ref, instance.uuid)
    ...
    def finish_revert_migration(self, context, instance, network_info,
        ...
        if self._is_legacy_in_place_migration(migration):
            ...
            self._do_finish_revert_migration(instance, block_device_info,
                                             network_info)

        if power_on:
            vm_util.power_on_instance(self._session, instance)

    def _do_finish_revert_migration(self, instance, block_device_info,
                                    network_info):
        vm_ref = vm_util.get_vm_ref(self._session, instance)
        ...
        self._attach_volumes(instance, block_device_info, adapter_type, vm_ref)
        # Reconfigure the networking
        self._add_vm_networking(vm_ref, instance, network_info)
    ...
    def _add_vm_networking(self, vm_ref, instance, network_info):
        client_factory = self._session.vim.client.factory
        config_spec = client_factory.create('ns0:VirtualMachineConfigSpec')
        extra_specs = self._get_extra_specs(instance.flavor,
                                            instance.image_meta)
    ...
    def _do_migrate_disk(self, context, vm_ref, instance, dest):
        ...
        cloned_vm = self._clone_vm(vm_ref, rel_spec, folder,
                                   name=self._get_migration_vm_name(instance))
        # keep track of the cloned vm by assigning migration.uuid to it
        migration = objects.Migration.get_by_id_and_instance(
            context, instance.migration_context.migration_id, instance.uuid)
        vm_util.change_vm_instance_uuid(session, cloned_vm, migration.uuid)
        if remote:
            session.logout()
    ...
    def _get_service_locator_spec(self, dest):
        ...
        service_locator.sslThumbprint = self._get_ssl_thumbprint(host, port)
    ...
    def _get_ssl_thumbprint(self, host, port):

...
class VMwareVMOps(object):
    ...
    def prep_cross_hv_conversion(self, context, instance):
        ...
        LOG.debug('Detached root disk %(path)s from instance '
            ...
            with self._virtapi.wait_for_instance_event(
                ...
                for port_id in port_ids:
                    if port_id not in port_realized_states:
                        LOG.warning("No realization status for port %s",
    ...
    def _do_confirm_migration(self, context, instance, migration):
        ...
        if not original_vm_ref:
            LOG.warning("Cannot find source vm for instance %s"
                        " in migration %s",
    ...
    def delete_config_drive_files(self, ctxt, instance, cdroms):
        for cdrom_spec in cdroms:
            try:
                ...
                # but we need some visibility here for the operator
    ...
    def _detach_volumes(self, instance, block_device_info):
        ...
        for disk in self._sort_disks(disks, reverse=True):
            ...
            try:
                ...
                LOG.warning("Cannot find disk %s."
                            " Assuming it to be removed", disk)
    ...
    def _destroy_expired_image_templates(self, templ_vm_folder_ref):
        ...
        for templ_vm_ref, templ_vm_name, ds_name in expired_templ_vms.values():
            ...
            with lockutils.lock(str(cache_image_path),
                                lock_file_prefix='nova-vmware-fetch_image'):
                LOG.debug("Destroying expired image-template VM %s",
                          templ_vm_name)
                try:
    ...
    def _filter_instances_for_drs(self, context, instance_uuids, sg_uuid=None,
        ...
        if sg_uuid:
            ...
            msg_prefix = "Excluding instance %s from VmGroup %s, "

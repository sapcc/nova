...
def _get_server_groups(context, instance, include_provider_groups=True):
    server_group_infos = []
    try:
    ...
    if include_provider_groups:
        _is_big_vm = is_big_vm(int(instance.memory_mb), instance.flavor)
        if CONF.vmware.bigvm_provider_vm_group and _is_big_vm:
            group = GroupInfo(CONF.vmware.bigvm_provider_vm_group, None)
            server_group_infos.append(group)
        elif CONF.vmware.non_bigvm_provider_vm_group and not _is_big_vm:
            group = GroupInfo(CONF.vmware.non_bigvm_provider_vm_group, None)
            server_group_infos.append(group)

...
class FlavorPermissionRule(base.NovaPersistentObject, base.NovaObject):
    """Restricts public flavors to specific domains and projects.
    ...
    'domain_id', 'project_id' and 'flavor_id'. Consequently, a public flavor is
    only denied at a scope if for the corresponding project:
    ...
    Note: Flavor permission rules only apply to public flavors. Private flavors
    are controlled exclusively via the flavor access list.
    """
    ...
    def _from_db_object(
        ...
        for field in rule.fields:
            ...
            if field in db_rule:
                ...
                if (field in DB_NONE_SENTINELS and
                        value == DB_NONE_SENTINELS[field]):
                    value = None
    ...
    def _create_in_db(
        ...
        db_rule = api_models.FlavorPermissionRule()
        db_values = FlavorPermissionRule._to_db_values(values)
        db_rule.update(db_values)
        try:
...
class FlavorPermissionRuleList(base.ObjectListBase, base.NovaObject):
    ...
    def _get_from_db(
        context: nova_context.RequestContext,
        filter_by_context_domain: bool = True,
        filter_by_context_project: bool = True,
        domain_id: str | None = None,
        project_id: str | None = None,
        scope: str | None = None,
        effect: str | None = None,
        flavor_id: int | None = None,
        has_flavor: bool | None = None,
        limit: int | None = None,
        marker: str | None = None,
    ) -> list[api_models.FlavorPermissionRule]:
        """Get flavor permission rules from the database.

        :param filter_by_context_domain: If True, restrict rules to
            'context.project_domain_id'.
        :param filter_by_context_project: If True, restrict rules to
            'context.project_id'.
        :param domain_id: Filter by 'domain_id'. None means no filter.
        :param project_id: Filter by 'project_id'. None means no filter.
        :param scope: Filter by 'scope' ('domain' or 'project').
        :param effect: Filter by 'effect' ('allow' or 'deny').
        :param flavor_id: Filter by 'flavor_id'. None means no filter.
        :param has_flavor: If True, return only flavor-specific rules. If
            False, return only default rules. None means no filter.
        ...
        query = context.session.query(Rule)
        if scope == fields.FlavorPermissionRuleScope.DOMAIN:
            query = query.filter(
                Rule.project_id == DB_NONE_SENTINELS['project_id'])
        elif scope == fields.FlavorPermissionRuleScope.PROJECT:
            query = query.filter(
                Rule.project_id != DB_NONE_SENTINELS['project_id'])

        if filter_by_context_domain:
            if not context.project_domain_id:
                return []
            query = query.filter(Rule.domain_id == context.project_domain_id)
        if filter_by_context_project:
            if not context.project_id:
                return []
            query = query.filter(Rule.project_id == context.project_id)
        if domain_id is not None:
            query = query.filter(Rule.domain_id == domain_id)
        if project_id is not None:
            query = query.filter(Rule.project_id == project_id)
        if effect is not None:
            query = query.filter(Rule.effect == effect)
        if flavor_id is not None:
            query = query.filter(
                Rule.flavor_id == flavor_id)

        if has_flavor is True:
            query = query.filter(
                Rule.flavor_id != DB_NONE_SENTINELS['flavor_id'])
        elif has_flavor is False:
            query = query.filter(
                Rule.flavor_id == DB_NONE_SENTINELS['flavor_id'])

        marker_rule = None
        if marker is not None:
            marker_query = context.session.query(Rule).filter_by(uuid=marker)
            marker_rule = marker_query.first()
            if not marker_rule:
                raise exception.MarkerNotFound(marker=marker)

        query = paginate_query(query, Rule, limit, ['id'], marker=marker_rule)
        return query.all()
    ...
    def get_all(
        ...
        db_rules = cls._get_from_db(
            context,
            filter_by_context_domain=filter_by_context_domain,
            filter_by_context_project=filter_by_context_project,
            domain_id=domain_id, project_id=project_id, scope=scope,
            effect=effect, flavor_id=flavor_id, has_flavor=has_flavor,

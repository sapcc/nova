...
class ComputeNodeTestCase(test.TestCase, ModelsObjectComparatorMixin):
    ...
    def test_compute_node_get_all_by_pagination_deleted_service(self):
        """ComputeNodes having a deleted service entry are not returned

        They get filtered out by our only caller in a way that breaks
        pagination,  so we filter them down on DB layer, too.
        """
        service_dict = dict(host='host2', binary='nova-compute',
                            topic=compute_rpcapi.RPC_TOPIC,
                            report_count=1, disabled=False)
        ...
        db.service_destroy(self.ctxt, service.id)
        # this ^  also deletes the compute node, so we undelete it through the
        # creation handler for duplicate entries
        compute_node = db.compute_node_create(self.ctxt, compute_node_dict)

        compute_node = db.compute_node_get(self.ctxt, compute_node.id)
        self.assertEqual(compute_node['deleted'], 0)

        nodes = db.compute_node_get_all_by_pagination(self.ctxt)
        self.assertEqual(len(nodes), 1)
        self.assertEqual(nodes[0]['host'], self.service_dict['host'])

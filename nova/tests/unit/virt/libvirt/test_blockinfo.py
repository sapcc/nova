...
class LibvirtBlockInfoTest(test.NoDBTestCase):
    ...
    def test_get_disk_bus_for_device_type_with_supported_buses(self):
        ...
        # Should pick first valid bus (virtio is valid for kvm)
        self.assertIn(bus, ['virtio', 'scsi'])
    ...
    def test_get_disk_bus_for_device_type_supported_buses_priority(self):
        ...
        bus = blockinfo.get_disk_bus_for_device_type(
            instance, 'kvm', image_meta, device_type='disk')
        # Should use hw_supported_disk_buses, not hw_disk_bus
        self.assertIn(bus, ['scsi', 'virtio'])
        self.assertNotEqual('ide', bus)

...
class FlavorPermissionRulesPolicyTest(base.BasePolicyTest):
    ...
    def _authorized_contexts(self, scope: str) -> list:
        return self.admin_authorized_contexts

    def _check_not_found_auth(self, authorized_contexts, func,
                              *args, **kwargs):
        """Check a policy where unauthorized access raises HTTPNotFound"""
        ...
        for ctx in unauth:
            self.req.environ['nova.context'] = ctx
            self.assertRaises(
                webob.exc.HTTPNotFound, func, self.req, *args, **kwargs)

    ...
    def _compare_obj(self, rule_obj, db_rule, db_allow_missing=None,
        ...
        for field in rule_obj.fields:
            ...
            if (field in DB_NONE_SENTINELS and
                    db_val == DB_NONE_SENTINELS[field]):
                db_val = None
...
class TestFlavorPermissionRuleObjectNoDB(test.NoDBTestCase,
    ...
    def test_scope_domain(self):
        ...
        self.assertEqual(
            fields.FlavorPermissionRuleScope.DOMAIN, rule.scope)

    def test_scope_project(self):
        rule = self._get_fake_rule_obj()
        self.assertEqual(
            fields.FlavorPermissionRuleScope.PROJECT, rule.scope)
...
class _TestFlavorPermissionRuleObjectDB(_TestFlavorPermissionRuleObject):
    ...
    def test_get_by_uuid_not_found(self):
        self.assertRaises(
            exception.FlavorPermissionRuleNotFound,
            objects.FlavorPermissionRule.get_by_uuid,
            self.context, 'nonexistent-uuid')

    def test_create_duplicate(self):
        fake_db_rule = self._fake_db_rule.copy()
        fake_db_rule.pop('id')
        rule1 = objects.FlavorPermissionRule(
            context=self.context, **fake_db_rule)
        rule1.create()
        rule2 = objects.FlavorPermissionRule(
            context=self.context, **fake_db_rule)
        self.assertRaises(
            exception.FlavorPermissionRuleExists, rule2.create)
...
class TestFlavorPermissionRuleListObjectNoDB(test.NoDBTestCase,
    ...
    @ddt.data(
        ({}, True, True),
        ({'filter_by_context_domain': False}, False, True),
        ({'filter_by_context_project': False}, True, False),
    )
    @ddt.unpack
    @mock.patch('nova.objects.FlavorPermissionRuleList._get_from_db')
    def test_get_all_context_filters(
            self, call_kwargs, exp_filter_domain, exp_filter_project,
            mock_get):
        ...
        mock_get.assert_called_once_with(
            self.context,
            filter_by_context_domain=exp_filter_domain,
            filter_by_context_project=exp_filter_project,
            domain_id=None, project_id=None, scope=None, effect=None,
            flavor_id=None, has_flavor=None, limit=None, marker=None)
...
class _TestFlavorPermissionRuleListObject(_TestFlavorPermissionRuleObject):
    ...
    def test_get_all_domain_and_project_filters(self):
        ...
        self.assertEqual(
            ...
            _get_all(filter_by_context_project=False))
        self.assertEqual(
            {('fake-domain', 'fake-project'),
             ('fake-domain', 'other-project')},
        ...
        self.assertEqual(
            {('fake-domain', None), ('fake-domain', 'fake-project'),
             ('fake-domain', 'other-project'),
             ('other-domain', 'other-project')},
            _get_all(filter_by_context_domain=False,
                     filter_by_context_project=False))

    def test_get_all_marker_not_found(self):
        self.assertRaises(
            ...
            filter_by_context_domain=False,
            filter_by_context_project=False,
            marker='nonexistent-uuid')

    def test_get_all_empty_context_domain(self):
        self._create_db_rule()
    ...
    def test_get_all_empty_context_project(self):
        self._create_db_rule()
        ctx = context.RequestContext(
            'fake-user', None, project_domain_id='fake-domain')
        rules = objects.FlavorPermissionRuleList.get_all(
            ctx,
            filter_by_context_domain=False,
    ...
    def test_get_all_effect_filter(self):
        allow_rule = self._create_db_rule()
        deny_rule = self._create_db_rule(
            self._get_fake_db_rule(
                project_id='other-project',
                effect=fields.FlavorPermissionRuleEffect.DENY))
        rules = objects.FlavorPermissionRuleList.get_all(
            self.context,
        ...
        self.assertEqual(1, len(rules))
        self.assertEqual(allow_rule.id, rules[0].id)
        rules = objects.FlavorPermissionRuleList.get_all(
            self.context,
            filter_by_context_domain=False,
            filter_by_context_project=False,
            effect=fields.FlavorPermissionRuleEffect.DENY)
        self.assertEqual(1, len(rules))
        self.assertEqual(deny_rule.id, rules[0].id)

    def test_get_all_flavor_id_filter(self):
        self._create_db_rule()
        flavor_id = 456
        rule = self._create_db_rule(
            self._get_fake_db_rule(flavor_id=flavor_id))
        rules = objects.FlavorPermissionRuleList.get_all(
            self.context,
            filter_by_context_domain=False,
            filter_by_context_project=False,
            flavor_id=flavor_id)
        self.assertEqual(1, len(rules))
        self.assertEqual(rule.id, rules[0].id)

    def test_get_all_has_flavor_filter(self):
        flavor_rule = self._create_db_rule()
        default_rule = self._create_db_rule(
            self._get_fake_db_rule(flavor_id=None))
        ...
        rules = objects.FlavorPermissionRuleList.get_all(
            ...
            has_flavor=False)
        self.assertEqual(1, len(rules))
        self.assertEqual(default_rule.id, rules[0].id)
        self.assertIsNone(rules[0].flavor_id)
        ...
        rules = objects.FlavorPermissionRuleList.get_all(
            ...
            has_flavor=True)
        self.assertEqual(1, len(rules))
        self.assertEqual(flavor_rule.id, rules[0].id)
        self.assertIsNotNone(rules[0].flavor_id)

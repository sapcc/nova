...
class GlanceImageServiceV2(object):
    ...
    def _verify_and_write(self, context, image_id, verifier,
        ...
        may_close_iterator = True

        try:
            # Exit early if we do not need write nor verify
            if verifier is None and not write_image:
                if data is None:
                    # If we return the iterator, then we should not close it
                    may_close_iterator = False
                    return image_chunks
                else:
                    return
            ...
            if close_file:
                ...
                data.close()
            if (isinstance(image_chunks, glance_utils.IterableWithLength) and
                    may_close_iterator):
